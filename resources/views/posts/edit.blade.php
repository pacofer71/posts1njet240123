<x-app-layout>
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <h5 class="my-2 text-xl text-center">Editar Post</h5>
            <div class="p-4 bg-gray-200 rounded-3xl shadow-2xl mx-auto w-1/2">
                <form action="{{ route('posts.update', $post) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <x-jet-input class="w-full p-2" name="titulo" id="titulo" placeholder="Título..." value="{{old('titulo', $post->titulo)}}" />
                    <x-jet-input-error for="titulo" /> 
                    <x-jet-label id="contenido" class="my-2">Contenido</x-jet-label>
                    <textarea name="contenido" rows='4' class="w-full" placeholder="Contenido...">{{old('contenido', $post->contenido)}}</textarea>
                    <x-jet-input-error for="contenido" /> 
                    <x-jet-label id="" class="my-2">Estado</x-jet-label>
                    <div class="flex">
                        <div class="form-check form-check-inline mr-4">
                            <input @checked(old('estado', $post->estado)=='Publicado')
                        ...
                        <div class="form-check form-check-inline">
                            <input @checked(old('estado', $post->estado)=='Borrador')
                                class="form-check-input form-check-input appearance-none rounded-full h-4 w-4 border border-gray-300 bg-white checked:bg-blue-600 checked:border-blue-600 focus:outline-none transition duration-200 mt-1 align-top bg-no-repeat bg-center bg-contain float-left mr-2 cursor-pointer"
                    ...
                    <div class="my-2">
                        <img src="{{ Storage::url($post->imagen) }}" class="w-1/2 object-cover object-center" alt="No Image"
                            id="img" />
                    </div>
